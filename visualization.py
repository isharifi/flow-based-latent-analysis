...
def interpolate(G, z, shifts_r, shifts_count, dim, deformator=None, with_central_border=False):
    ...
    for shift in np.arange(-shifts_r, shifts_r + 1e-9, shifts_r / shifts_count):
        ...
        latent_shift = latent_shift.view([-1] + G.dim_z)
        shifted_image = G(z=z + latent_shift, reverse=True).cpu()[0].clamp_(0, 1)
...
@torch.no_grad()
def make_interpolation_chart(G, deformator=None, z=None, std=0.5,
    ...
    if with_deformation:
        ...
        deformator.eval()
    z = z if z is not None else make_noise(1, G.dim_z, std).cuda()

    if with_deformation:
        original_img = G(z=z, reverse=True).cpu()
    else:
        original_img = G(z=z, reverse=True).cpu()
    imgs = []
    if dims is None:
...
@torch.no_grad()
def make_interpolation_chart_per_dimension(G, deformator=None, zs=None, std=0.5,
                                           shifts_r=10.0, shifts_count=5,
                                           dims=None, dims_count=5, texts=None, **kwargs):
    with_deformation = deformator is not None
    if with_deformation:
    ...
    for z in zs:
        z = z.unsqueeze(0)
        imgs.append(interpolate(G, z, shifts_r, shifts_count, dims[0], deformator))
    ...
    for ax, shifts_imgs in zip(axs[0:], imgs):
        ax.axis('off')
        plt.subplots_adjust(left=0.1)
        ax.imshow(to_image(make_grid(shifts_imgs, nrow=(2 * shifts_count + 1), padding=1), True))
        # ax.text(-20, 21, str(text), fontsize=10)
...
@torch.no_grad()
def inspect_all_directions(G, deformator, out_dir, directions_count, std=0.5, zs=None, num_z=3, shifts_r=8.0):
    os.makedirs(out_dir, exist_ok=True)

    step = 5  # 20
    max_dim = directions_count
    zs = zs if zs is not None else make_noise(num_z, G.dim_z, std).cuda()
    ...
    for start in range(0, max_dim - 1, step):
        ...
        for z in zs:
            ...
            fig = make_interpolation_chart(
                G, deformator=deformator, z=z, std=std,
...
@torch.no_grad()
def inspect_all_directions_per_direction(G, deformator, out_dir, directions_count, std=0.5, zs=None, num_z=20,
                                         shifts_r=8.0):
    os.makedirs(out_dir, exist_ok=True)
    ...
    # for start in range(0, max_dim - 1, step):
    for start in range(0, max_dim, step):
        print(f"[Visualization]: Direction {start}")
        dims = range(start, min(start + step, max_dim))
        zs = make_noise(num_z, G.dim_z, std).cuda()
        shifts_count = 3

        fig = make_interpolation_chart_per_dimension(
            G, deformator=deformator, zs=zs, std=std,
            shifts_count=shifts_count, dims=dims, shifts_r=shifts_r,
            dpi=700)
        out_file = os.path.join(out_dir, f'{dims[0]}.jpg')
        fig_to_image(fig).convert("RGB").save(out_file)
